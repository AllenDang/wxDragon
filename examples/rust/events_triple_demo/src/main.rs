...
fn main() {
    ...
    let _ = wxdragon::main(|_| {
        ...
        {
            let idle_remaining = idle_remaining.clone();
            let status = status.clone();
            idle_btn.on_click(move |_| {
                *idle_remaining.borrow_mut() = 100000; // units of work
        ...
        // Idle handler: process a few units each idle; also apply background messages
        {
            ...
            frame.on_idle(move |e| {
                ...
                let mut left = idle_remaining.borrow_mut();
                if *left > 0 {
                    let step = 7;
