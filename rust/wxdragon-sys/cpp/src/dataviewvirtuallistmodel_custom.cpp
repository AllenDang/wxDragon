...
public:
    typedef void (*GetValueCallback)(void* userdata, uint64_t row, uint64_t col,
                                     wxd_Variant_t* variant);
    typedef bool (*SetValueCallback)(void* userdata, const wxd_Variant_t* variant, uint64_t row,
                                     uint64_t col);
    typedef bool (*GetAttrCallback)(void* userdata, uint64_t row, uint64_t col,
    ...
    WxdCustomDataViewVirtualListModel(unsigned int initial_size, void* userdata,
                                      GetValueCallback get_value, SetValueCallback set_value,
                                      GetAttrCallback get_attr, IsEnabledCallback is_enabled)
    ...
    GetValueByRow(wxVariant& variant, unsigned int row, unsigned int col) const override
    {
        ...
        case 2:
        case 8:
            if (rust_variant_data.type == WXD_VARIANT_TYPE_STRING &&
                rust_variant_data.data.string_val) {
        ...
        case 5:
                    ...
                    if (casted_bmp->IsOk()) {
                        try {
                            ...
                            if (final_copy.IsOk()) {
                                ...
                                wxBitmap fallback_bmp_on_error(16,
                                                               16); /* Create a visible fallback */
                            ...
                            WXD_LOG_ERROR(
                                "GetValueByRow: Exception during wxBitmap copy from borrowed ptr for WXD_VARIANT_TYPE_BITMAP_RUST_BORROWED.");
                            wxBitmap fallback_bmp_on_exception(16,
                                                               16); /* Create a visible fallback */
            ...
            else if (rust_variant_data.type ==
                     WXD_VARIANT_TYPE_BITMAP) { // Handle OLD type (should ideally not be used by Rust anymore for get_value)
    ...
    SetValueByRow(const wxVariant& variant, unsigned int row, unsigned int col) override
    {
        if (m_set_value) {
            ...
            if (type_name == "bool") {
                ...
                rust_variant.data.int64_val =
                    static_cast<int64_t>(variant.GetLongLong().GetValue());
            }
            ...
            bool result = m_set_value(m_userdata, &rust_variant, static_cast<uint64_t>(row),
