...
pub fn create_advanced_tab(notebook: &Notebook) -> (SplitterWindow, AdvancedTabControls) {
    ...
    let tree_status_label = StaticText::builder(&tree_panel).with_label("Tree Selection: None").build();
    ...
    // Add a title
    let gauge_title = StaticText::builder(&gauge_panel).with_label("Gauge Demo").build();
    gauge_sizer.add_spacer(10);
    gauge_sizer.add(&gauge_title, 0, SizerFlag::AlignCenterHorizontal | SizerFlag::All, 5);
    ...
    let gauge_increase_btn = Button::builder(&gauge_panel).with_label("Increase (+10)").build();
    ...
    // Gauge status label
    let gauge_status_label = StaticText::builder(&gauge_panel).with_label("Gauge Value: 25%").build();
    gauge_sizer.add(&gauge_status_label, 0, SizerFlag::AlignCenterHorizontal | SizerFlag::All, 5);
    ...
    slider_sizer.add(&slider_title, 0, SizerFlag::AlignCenterHorizontal | SizerFlag::All, 5);
    ...
    let spin_title = StaticText::builder(&spin_panel).with_label("SpinCtrl Demo").build();
    spin_sizer.add_spacer(10);
    spin_sizer.add(&spin_title, 0, SizerFlag::AlignCenterHorizontal | SizerFlag::All, 5);
    ...
    let spin_ctrl = SpinCtrl::builder(&spin_panel)
        ...
        .with_max_value(1000)
        .build();
    spin_sizer.add(&spin_ctrl, 0, SizerFlag::AlignCenterHorizontal | SizerFlag::All, 5);

    // SpinCtrl status label
    let spin_ctrl_label = StaticText::builder(&spin_panel)
        .with_label(&format!("Spin Value: {INITIAL_VALUE}"))
        .build();
    spin_sizer.add(&spin_ctrl_label, 0, SizerFlag::AlignCenterHorizontal | SizerFlag::All, 5);
...
fn add_dnd_demo(panel: &Panel, sizer: &BoxSizer) {
    // Add a title
    let title = StaticText::builder(panel).with_label("Drag and Drop Demo").build();
    sizer.add_spacer(10);
    ...
    let source_text = StaticText::builder(&source_panel).with_label("Drag from here").build();
    ...
    let target_label = StaticText::builder(&target_panel).with_label("Drop here").build();
    ...
    let file_drop_title = StaticText::builder(panel).with_label("File Drop Demo").build();
    sizer.add_spacer(10);
    ...
    let file_target_label = StaticText::builder(&file_target_panel).with_label("Drop files here").build();
