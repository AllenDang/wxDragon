...
pub fn create_book_controls_tab(notebook: &Notebook) -> BookControlsTab {
    let tab_panel = Panel::builder(notebook).with_style(PanelStyle::TabTraversal).build();
    ...
    let settings_label = StaticText::builder(&settings_page_panel)
        ...
        .build();
    let settings_button = Button::builder(&settings_page_panel).with_label("Settings Button").build();
    ...
    BookControlsTab { tab_panel, treebook }
}
